...
gw_mat_ar = matrix(NA, 12, 2)
gw_mat_arma = matrix(NA, 12, 2)

# AR

for (hor in 1:12) {
  load("UK/forecasts/ar.rda")
  y_bench = forecasts[, hor]
  ...
  gw_mat_ar[hor, 1] = gw$statistic
  gw_mat_ar[hor, 2] = "*"
  
  if (gw$p.value <= 0.0001) {
    gw_mat_ar[hor, 2] = "****"
  }
  else if (gw$p.value <= 0.001) {
    gw_mat_ar[hor, 2] = "***"
  }
  else if (gw$p.value <= 0.01) {
    gw_mat_ar[hor, 2] = "**"
  }
  else if (gw$p.value <= 0.05) {
    gw_mat_ar[hor, 2] = "*"
  }
  else if (gw$p.value <= 0.1) {
    gw_mat_ar[hor, 2] = "."
  }
  else {
    gw_mat_ar[hor, 2] = ""
